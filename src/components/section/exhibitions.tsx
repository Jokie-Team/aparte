...
const Section: React.FC<SectionProps> = ({
  ...
  const formatDate = (date?: string) =>
    dateFmt.format(new Date(date ?? Date.now()));
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-row justify-between ${isImageRight ? "" : "flex-row-reverse"}`}
      >
        ...
        <div className="flex flex-col space-y-4 w-full md:w-1/2">
          ...
          {(exhibitionDetails.description?.length ?? 0) >
            MAX_NO_CHARACTERS_DESCRIPTION && (
            <span
