...
const Menu: React.FC<MenuProps> = async ({ isMobileMenuOpen, toggleMobileMenu , lng}) => {
    ...
    const genreateLanguageSwitchLink = ()=> {
        if (lng === 'en') {
        ...
        <div className="h-full">
            <nav className="hidden md:block">
                <ul className="flex pt-10 gap-10 lg:gap-40">
                    <li className='h-10 flex flex-row items-center cursor-pointer' onClick={() => handleNavigation('/exhibitions')}>{t("expositions")}</li>
                    ...
                    <li className='h-10 flex flex-row items-center'><Link href={genreateLanguageSwitchLink()} className='"hover:text-gray-900"'>{lng === 'en' ? "PT" : "EN"}</Link></li>
                </ul>
            </nav>
